...
8. If explicitly asked to start a new case, use host tools only inside the exact
   user-selected, host-granted `قانونك/` root; do not overwrite, move, merge, or
...
versions are incompatible, the host lacks the selected root, or the connector
would be written through. Do not guess or present a draft after a failed proof.
